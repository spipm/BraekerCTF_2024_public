...
def create_profile():
	...
	sql = '''INSERT INTO profiles (username, stuff) VALUES (?,?)'''
	cur = conn.cursor()
	cur.execute(sql, (new_username, ''))
	conn.commit()
...
# select stuffs from 20 users
def get_latest_stuff():

	sql = "SELECT username,stuff from profiles WHERE stuff not LIKE ? ORDER BY id DESC LIMIT 20"
	cur = conn.cursor()
	cur.execute(sql, ["%%%s%%" % flag])

	stuff = cur.fetchmany(size=20)
...
def set_stuff():
	...
	if username and stuff and special_type and special_val:
		...
		post_body = 'api_key=%s&username=%s&stuff=%s' % (flag,username,stuff)
